    ...
    $.extend(TreeGrid.prototype, {
        fnConstruct: function (oInit) {
            ...
            dataTable.on('click', 'td.treegrid-control-open', function (e) {
                ...
                var childs = getChildrenCollapseIndexs2(dataTable, index);/*fix:index ary @autor:sdjkjkjjk*/
...
console.info("getChildrenCollapseIndexs", index);

        var ret = [];
        var $filted = $(dataTable.rows('[parent-index="' + index + '"]').nodes());

        console.info("$filted", $filted);
...
console.info("ary=", ary);
            ...
            console.info("$.extend前 $filted=", $filted);
            console.info("$.extend前 ary=", ary);

            $.extend($filted, ary, true);

            console.info("$.extend後 $filted=", $filted);
            console.info("$.extend後 ary=", ary);
    ...
    function getChildrenCollapseIndexs2(dataTable, index) {
